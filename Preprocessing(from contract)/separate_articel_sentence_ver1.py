...
contract_name = '24년 개정 직매입 표준거래계약서(면세점)'

#######################################################################################################################
# Contract to Article
#######################################################################################################################
def contract_to_article(text):
    pattern = r'(제\d+조(?!\S))'
...
with open(f'./Data_Analysis/Contract/{contract_name}.txt', 'r', encoding='utf-8') as file:
    text = file.read()

contract_article_1 = contract_to_article(text)

contract_article_2 = update_article_keys(contract_article_1)

# ✅ JSON 파일로 저장
json_path = f"./Data_Analysis/Data/contract_to_article_({contract_name}).json"
with open(json_path, 'w', encoding='utf-8') as json_file:
    json.dump(contract_article_2, json_file, ensure_ascii=False, indent=4)
...
with open(json_path, 'r', encoding='utf-8') as json_file:
    contract_articles = json.load(json_file)
...
split_df.to_csv('./Data_Analysis/Data/contract_to_sentences.csv', index=False, encoding='utf-8-sig')
